...
class Utils {
  ...
  getPageTitle = (pathname: string) => {
    ...
    const localizedAppName = 'NAWASCO APP';
  ...
  getNumberFromString(value: string | undefined | any){
  ...
  formatNumber(value: any){
    if (value ==='' || value ===undefined || value ===null) {
      ...
      var number = this.getNumberFromString(value);
      return this.numberToLocaleString(number)
    }
  }
  numberToLocaleString(value: any, minimumFractionDigits = 2){
    if (value ==='' || value ===undefined || value ===null) {
      return 0
    } else {
      return Number(value).toLocaleString('vi-VI', { minimumFractionDigits: minimumFractionDigits })
    }
  }
}
